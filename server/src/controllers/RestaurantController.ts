...
export default class RestaurantController implements Crud{
    ...
    async create(request: Request, response: Response){
        const {name, adress, foodType} = request.body;

        const isAnyUndefined = Citi.areValuesUndefined(name, adress, foodType);
        if(isAnyUndefined) return response.status(400).send();

        const newRestaurant = {name, adress, foodType};
    ...
    async update(request: Request, response: Response){
        const {id} = request.params;
        const {name, adress, foodType} = request.body;
        const isAnyUndefined = Citi.areValuesUndefined(name, adress, foodType, id);
        if(isAnyUndefined) return response.status(400).send();

        const restaurantWithUpdatedValues = {name, adress, foodType};
